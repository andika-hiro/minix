...
export function DirectMessages({ currentUser }: DirectMessagesProps) {
  ...
  const recipients = useMemo(() => {
    ...
    return activeConversation.participant_profiles.filter(
  ...
  async function loadConversations(user: User, preferredId?: string | null) {
    ...
    try {
      ...
      const r = await fetch(
        `${DM_API_BASE}/dm_conversations?${params.toString()}`
      );
  ...
  async function loadMessages(conversationId: string) {
    ...
    try {
      const r = await fetch(
        `${DM_API_BASE}/dm_conversations/${conversationId}/messages`
      );
      const json = await r.json();
  ...
  async function handleSend() {
    ...
    try {
      await fetch(
        `${DM_API_BASE}/dm_conversations/${selectedConversationId}/messages`,
        {
          method: 'POST',
  ...
  async function handleCreateConversation() {
    ...
    try {
      const r = await fetch(
        `${DM_API_BASE}/dm_conversations/${target}/messages`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      setNewMessage('');
      setNewTarget('');
      const nextSelection = await loadConversations(
        currentUser,
        conversationId ?? null
      );
    ...
    return (
      <div>
        ...
        <p className='text-muted'>
          Login to send DMs with the mock Twitter API.
        </p>
      </div>
    );
  }
          ...
          <div className='dm-list'>
            {loadingList && conversations.length === 0 ? (
              ...
              conversations.map((conversation) => {
                const recipient = conversation.participant_profiles?.find(
          ...
          <div className='dm-thread'>
            {selectedConversationId && activeConversation ? (
              <>
                ...
                <div
                  className={`dm-messages ${loadingThread ? 'loading' : ''}`}
                >
                  {loadingThread ? (
